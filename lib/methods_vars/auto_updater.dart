...
Future<ReleaseInfo> getLatestRelease() async {
  try {
    ...
    Uri url = Uri.parse(
        'https://api.github.com/repos/tpkowastaken/autojidelna/releases/latest');
    ...
    for (int i = 0; i < json['assets'].length; i++) {
      String apkname =
          json['assets'][i]['browser_download_url'].split('/').last;
      if (apkname.contains('.apk')) {
        apkname.replaceAll('.apk', '');
      } else {
        patchNotes = utf8.decode((await http
                .get(Uri.parse(json['assets'][i]['browser_download_url'])))
            .bodyBytes);
      }
      for (int k = 0; k < androidInfo.supportedAbis.length; k++) {
        ...
        if (apkname.contains(androidInfo.supportedAbis[k]) ||
            i == json['assets'].length - 2) {
