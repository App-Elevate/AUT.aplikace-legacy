  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        ...
        body: const JidelnicekDenWidget(),
...
class JidelnicekDenWidget extends StatefulWidget {
  const JidelnicekDenWidget({super.key});

  @override
  State<JidelnicekDenWidget> createState() => JidelnicekDenWidgetState();
}

class JidelnicekDenWidgetState extends State<JidelnicekDenWidget> {
  DateTime date = DateTime.now();
  CanteenData canteenData = getCanteenData();
  ...
  Widget build(BuildContext context) {
    CanteenData canteenData = getCanteenData();
    String dayOfWeek = ziskatDenZData(date.weekday);
    return Scaffold(
      appBar: AppBar(
              ...
              ElevatedButton(
                onPressed: () async {
                  var datePicked = await showDatePicker(
                    ...
                    firstDate: DateTime(2006, 5, 23),
                    lastDate: date.add(const Duration(days: 365 * 2)),
                  );
                  if (datePicked == null) return;

                  preIndexLunches(datePicked, 7).then((_) => preIndexLunches(
                      datePicked.subtract(const Duration(days: 7)), 7));
                  setState(() {
                    date = datePicked;
                  });
                },
                child: Text(
                    "${date.day}. ${date.month}. ${date.year} - $dayOfWeek"),
              ),
              Text('kredit: ${canteenData.uzivatel.kredit.round()}Kč'),
            ],
          )),
      body: FutureBuilder(
          future: getLunchesForDay(date),
          builder: (context, snapshot) {
...
class ListJidel extends StatelessWidget {
  ...
  final JidelnicekDenWidgetState widget;
